...
⚙ Settings → Model Services → pick a provider → **Import key…** → **Test connection** → **Set as active engine**. Keys are stored in the macOS Keychain (or a `chmod 600` file elsewhere), never in `providers.json`, backups, diagnostics bundles, or telemetry.
