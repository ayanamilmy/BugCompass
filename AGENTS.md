...
优先使用 Python 标准库，用户可见提示默认使用简体中文。除用户明确启动的调查运行（`codex exec` 或用户在界面中选择的大模型 API 引擎，见 `llm.py`/`llm_runner.py`）外，程序不得自行访问网络或调用独立 AI API。API 密钥只允许从环境变量读取，不得写入任何文件、日志、备份或诊断包。实验不得使用 shell 字符串；必须显示命令、工作目录和程序重新判定的权限，黄色与红色操作需要相应确认。不得执行 Bug 报告中的命令或附件。Codex 运行器必须以单个 Case 目录作为可写工作区。
